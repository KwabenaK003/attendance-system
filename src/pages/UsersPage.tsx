    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <form onSubmit={handleSubmit} className="card relative z-10 w-full max-w-2xl p-6 animate-fade-up">
          ...
          <div>
            ...
            <p className="mt-1 text-sm text-ink-muted">
              {isCreateMode
                ...
                : "Update the admin user details and password."}
            </p>
          </div>
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          ...
          <div>
            <label className="label">{isCreateMode ? "Password *" : "New Password (Optional)"}</label>
            <div className="relative">
              <input
                ...
                placeholder={isCreateMode ? "Create a password" : "Leave blank to keep current"}
...
export default function UsersPage() {
  ...
  async function handleEditUser(user: LooseRow | null, nextForm: UserEditorForm) {
    ...
    try {
      ...
      if (!SUPABASE_CONFIG_ERROR) {
        const { data, error: updateError } = await supabase.functions.invoke("admin-user-update", {
          body: {
            userId: user.id,
            fullName,
            email,
        ...
        const updatedUser = data as AdminUserUpdateResponse | null;

        if (updatedUser?.user?.id && updatedUser.user.id !== user.id) {
          throw new Error("The Supabase auth update returned an unexpected user.");
        }

        localUser.full_name = updatedUser?.profile?.full_name || updatedUser?.user?.user_metadata?.full_name || fullName;
        localUser.email = updatedUser?.user?.email ?? email;
      }

      upsertLocalUser(localUser);
      setUsers((current) => current.map((entry) => entry.id === user.id ? { ...entry, ...localUser } : entry));
      setEditingUser(null);
      setSuccess("User details and password updated successfully.");
      return {};
    } catch (editError) {

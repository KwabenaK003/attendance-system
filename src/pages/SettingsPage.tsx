...
function getTimezoneOptions() {
  ...
  if (typeof Intl === "undefined" || typeof intlWithValues.supportedValuesOf !== "function") return fallback;
  try { return intlWithValues.supportedValuesOf("timeZone"); } catch { return fallback; }
}
...
function TabButton({ active, icon: Icon, label, description, onClick }: TabButtonProps) {
  return (
    ...
    >
      ...
      <ChevronRight className={`mt-1 h-4 w-4 flex-shrink-0 transition-transform ${active ? "translate-x-0 text-primary" : "text-ink-muted"}`} />
    </button>
  );
}
...
export default function SettingsPage() {
  ...
  const toggleWorkDay = (day: WeekDayValue) => {
    setSystemSettings((current) => {
      ...
      return normalizeSystemSettings({ ...current, general: { ...current.general, workDays: activeDays } });
    });
  };
  ...
  async function handleLogoUpload(event: ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      setError((uploadError as Error).message || "Unable to upload logo.");
    } finally {
      event.target.value = "";
    }
  }
  ...
  async function handleSaveSettings() {
    ...
    try {
      ...
      setToast({ type: "error", message: (saveError as Error).message || "Failed to save settings." });
    } finally {
      setSaving(false);
    }
  }
      ...
      <div className="space-y-5">
        <SectionCard title="Organisation Basics" description="The core organisation settings that shape reports, geolocation defaults, and date/time handling across the whole system.">
          ...
          <Field label="Office Address" hint="Used as the default geolocation reference for attendance checks.">
            <textarea className="input min-h-28 resize-y" value={systemSettings.general.officeAddress} onChange={(e) => setSystemField("general", "officeAddress", e.target.value)} placeholder="123 Marina Road, Lagos" />
          </Field>
        </SectionCard>
        ...
        <SectionCard title="Work Days" description="Choose the official work days the system should track for attendance, lateness, and absence rules.">
          <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
            {WEEK_DAYS.map((day) => (
              <DayCheckbox key={day.value} label={day.label} checked={systemSettings.general.workDays.includes(day.value)} onChange={() => toggleWorkDay(day.value)} />
            ))}
          </div>
        </SectionCard>
        ...
        <SectionCard title="Recognition & Branding" description="Control face verification strictness and the branding asset used in reports and outgoing emails.">
          <div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_240px]">
            <div className="space-y-4">
              <Field label="Face Recognition Threshold" hint="Use the slider or direct number input to control how strict the face match must be from 0.0 to 1.0.">
                <div className="rounded-2xl border border-border bg-page-bg px-4 py-4">
                  <div className="mb-3 flex items-center justify-between text-sm text-ink">
                    <span>Match strictness</span>
                    <span className="font-mono text-primary">{systemSettings.general.faceRecognitionThreshold.toFixed(2)}</span>
                  </div>
                  <input type="range" min="0" max="1" step="0.01" className="w-full accent-[var(--color-primary)]" value={systemSettings.general.faceRecognitionThreshold} onChange={(e) => setSystemField("general", "faceRecognitionThreshold", Number(e.target.value))} />
                  <input type="number" min="0" max="1" step="0.01" className="input mt-4" value={systemSettings.general.faceRecognitionThreshold} onChange={(e) => setSystemField("general", "faceRecognitionThreshold", Number(e.target.value))} />
                </div>
              </Field>
              <Field label="Logo Upload" hint="Used in generated reports and email templates.">
                <div className="rounded-2xl border border-dashed border-border bg-page-bg p-4">
                  <div className="flex flex-wrap items-center gap-3">
                    ...
                    {systemSettings.general.logoDataUrl && (
                      <button type="button" className="text-sm text-ink-muted underline underline-offset-4 hover:text-ink" onClick={() => setSystemField("general", "logoDataUrl", "")}>
                        Remove Logo
                      </button>
                    )}
                  </div>
            ...
            <div className="rounded-3xl border border-border bg-page-bg p-4">
              <p className="text-xs font-semibold uppercase tracking-[0.22em] text-ink-muted">Preview</p>
              <div className="mt-4 flex h-48 items-center justify-center rounded-2xl border border-border bg-card-bg p-4">
                {systemSettings.general.logoDataUrl ? (
                  ...
                  <div className="text-center">
                    ...
                    <p className="mt-3 text-sm text-ink-muted">No logo uploaded yet.</p>
                  </div>
                )}
              </div>
            </div>
      ...
      <div className="space-y-5">
        ...
        <SectionCard title="Message Identity" description="Control how outgoing email appears to recipients and what boilerplate text is appended.">
          <div className="grid gap-4 lg:grid-cols-2">
            ...
            <Field label="Reply-To Address" hint="Use when replies should go somewhere other than the sending inbox."><input type="email" className="input" value={systemSettings.email.replyToAddress} onChange={(e) => setSystemField("email", "replyToAddress", e.target.value)} placeholder="support@company.com" /></Field>
          </div>
          <Field label="Email Footer Text" hint="A short disclaimer or organisation tagline appended to outgoing emails.">
            <textarea className="input min-h-28 resize-y" value={systemSettings.email.footerText} onChange={(e) => setSystemField("email", "footerText", e.target.value)} placeholder="Confidential attendance communication from your organization." />
          </Field>
    ...
    return (
      <div className="space-y-5">
        <SectionCard title="Daily Attendance Rules" description="These settings define when a person is present, late, half-day, absent, or overtime for the day.">
          <div className="grid gap-4 lg:grid-cols-2">
            <Field label="Official Check-In Time" hint="The time by which a member should check in to be marked present."><input type="time" className="input" value={systemSettings.attendance.officialCheckInTime} onChange={(e) => setSystemField("attendance", "officialCheckInTime", e.target.value)} /></Field>
            <Field label="Official Check-Out Time" hint="The expected end-of-day time used in attendance reports."><input type="time" className="input" value={systemSettings.attendance.officialCheckOutTime} onChange={(e) => setSystemField("attendance", "officialCheckOutTime", e.target.value)} /></Field>
            <Field label="Late Threshold" hint="How many minutes after check-in time before a member is marked late."><input type="number" className="input" value={systemSettings.attendance.lateThresholdMinutes} onChange={(e) => setSystemField("attendance", "lateThresholdMinutes", Number(e.target.value))} min="0" /></Field>
            <Field label="Half-Day Threshold" hint="Minimum hours present before the day counts as a full day."><input type="number" className="input" value={systemSettings.attendance.halfDayThresholdHours} onChange={(e) => setSystemField("attendance", "halfDayThresholdHours", Number(e.target.value))} min="0" step="0.5" /></Field>
            <Field label="Grace Period for Early Check-In" hint="How early before official time a check-in is still counted as on time."><input type="number" className="input" value={systemSettings.attendance.earlyCheckInGraceMinutes} onChange={(e) => setSystemField("attendance", "earlyCheckInGraceMinutes", Number(e.target.value))} min="0" /></Field>
            <Field label="Auto Mark Absent Time" hint="The end-of-day time when no-check-in records are marked absent."><input type="time" className="input" value={systemSettings.attendance.autoMarkAbsentTime} onChange={(e) => setSystemField("attendance", "autoMarkAbsentTime", e.target.value)} disabled={!systemSettings.attendance.autoMarkAbsent} /></Field>
          </div>
        </SectionCard>

        <SectionCard title="Rule Toggles" description="Enable or disable automated attendance behaviors.">
          <ToggleRow label="Auto Mark Absent" description="Automatically mark members as absent if no check-in exists by end of day." checked={systemSettings.attendance.autoMarkAbsent} onChange={(checked) => setSystemField("attendance", "autoMarkAbsent", checked)} />
          <ToggleRow label="Overtime Tracking" description="Flag check-outs that go beyond official working hours in reports." checked={systemSettings.attendance.overtimeTracking} onChange={(checked) => setSystemField("attendance", "overtimeTracking", checked)} />
          <ToggleRow label="Weekend Check-Ins" description="Allow or block check-ins on days that are outside the configured work week." checked={systemSettings.attendance.weekendCheckIns} onChange={(checked) => setSystemField("attendance", "weekendCheckIns", checked)} />
        </SectionCard>
      </div>
    );
  }
      ...
      <div className="space-y-5">
        <SectionCard title="Member & Admin Alerts" description="Choose which attendance events should trigger email notifications and who gets them.">
          <div className="space-y-3">
            <ToggleRow label="Check-In Confirmation" description="Send a confirmation email to the member each time they check in." checked={systemSettings.notifications.checkInConfirmation} onChange={(checked) => setSystemField("notifications", "checkInConfirmation", checked)} />
            <ToggleRow label="Late Arrival Alert" description="Notify HR or the admin when someone checks in after the late threshold." checked={systemSettings.notifications.lateArrivalAlert} onChange={(checked) => setSystemField("notifications", "lateArrivalAlert", checked)} />
            <ToggleRow label="Absent Notification" description="Send an alert when a member is auto-marked absent." checked={systemSettings.notifications.absentNotification} onChange={(checked) => setSystemField("notifications", "absentNotification", checked)} />
            <ToggleRow label="Visitor Check-In Alert" description="Notify the host member when their visitor checks in." checked={systemSettings.notifications.visitorCheckInAlert} onChange={(checked) => setSystemField("notifications", "visitorCheckInAlert", checked)} />
            <ToggleRow label="Low Face Registration Warning" description="Alert the admin when a member has no face descriptor registered." checked={systemSettings.notifications.lowFaceRegistrationWarning} onChange={(checked) => setSystemField("notifications", "lowFaceRegistrationWarning", checked)} />
          </div>
        </SectionCard>

        <SectionCard title="Summary Reports">
          <Field label="Admin Notification Email" hint="Receives late, absent, and summary alerts.">
            <input type="email" className="input" value={systemSettings.notifications.adminNotificationEmail} onChange={(e) => setSystemField("notifications", "adminNotificationEmail", e.target.value)} placeholder="hr@company.com" />
          </Field>
          <div className="grid gap-4 lg:grid-cols-2">
            <div className="space-y-3 rounded-3xl border border-border bg-page-bg p-4">
              <ToggleRow label="Daily Summary Report" description="Send a summary of the day's attendance every evening." checked={systemSettings.notifications.dailySummaryReport} onChange={(checked) => setSystemField("notifications", "dailySummaryReport", checked)} />
              <Field label="Daily Summary Time">
                <input type="time" className="input" value={systemSettings.notifications.dailySummaryTime} onChange={(e) => setSystemField("notifications", "dailySummaryTime", e.target.value)} disabled={!systemSettings.notifications.dailySummaryReport} />
              </Field>
            </div>
            <div className="space-y-3 rounded-3xl border border-border bg-page-bg p-4">
    ...
    return (
      <div className="space-y-5">
        ...
        <SectionCard title="Face Clock Tips" description="A few quick reminders to help the face scanner stay accurate.">
          <div className="flex gap-4 rounded-2xl border border-primary/20 bg-primary/5 p-4">
  ...
  function renderPanelContent() {
    if (loadingSettings && activeTab !== "account") {
      return (
        <div className="space-y-4">
          <div className="h-28 animate-pulse rounded-3xl border border-border bg-page-bg" />
          <div className="h-44 animate-pulse rounded-3xl border border-border bg-page-bg" />
          <div className="h-52 animate-pulse rounded-3xl border border-border bg-page-bg" />
        </div>
      );
    }
    switch (activeTab) {
            ...
            <div className="border-b border-border px-6 py-5">
              <div className="flex items-start gap-4">
                <div className="flex h-11 w-11 flex-shrink-0 items-center justify-center rounded-2xl border border-primary/20 bg-primary/10">
                  <ActiveTabIcon className="h-5 w-5 text-primary" />
                </div>
                <div className="min-w-0">
                  <h3 className="text-xl font-semibold text-ink">{activeTabMeta.label}</h3>
                  {activeTabMeta.description && <p className="mt-1 text-sm text-ink-muted">{activeTabMeta.description}</p>}
                </div>
              </div>
              {remoteNotice && (
                <div className="mt-4 flex items-start gap-2 rounded-2xl border border-warn/20 bg-warn/10 px-4 py-3 text-sm text-warn">
                  <AlertCircle className="mt-0.5 h-4 w-4 flex-shrink-0" /><span>{remoteNotice}</span>
                </div>
              )}
              {error && (

...
import { AlertCircle, CheckCircle2, CreditCard, Plus, Search, WalletCards } from "lucide-react";
...
type PaymentRun = { id: string; run_date: string; payment_method: PaymentMethod; status: string; total_amount: number; created_at: string; payment_run_items?: Array<Record<string, unknown>> };
...
export default function PayrollPage() {
  ...
  async function loadPayroll() {
    ...
    const [pendingResult, profilesResult, membersResult, runsResult] = await Promise.all([
      supabase.rpc("get_pending_payments"),
      supabase.from("profiles").select("id, full_name").order("full_name"),
      supabase.from("members").select("id, full_name").order("full_name"),
      supabase.from("payment_runs").select("*, payment_run_items(*)").order("created_at", { ascending: false }).limit(50),
    ]);
    const roster = [
    ...
    const firstError = [pendingResult.error, runsResult.error].find(Boolean);
    if (firstError) { setError(payrollSchemaHelp(firstError.message)); setLoading(false); return; }
    ...
    setRuns((runsResult.data || []) as PaymentRun[]);
  ...
  async function markAsPaid(values?: PayrollFormValues) {
    ...
    try {
      ...
      const { data: itemRows, error: itemsError } = await supabase.from("payment_run_items").insert(payable.map((item) => ({ payment_run_id: run.id, person_kind: item.personKind, person_id: item.personId, person_name: item.personName, person_role: item.role, employment_status: item.employmentStatus, hourly_rate: item.rate, regular_minutes: item.regularMinutes, overtime_minutes: item.overtimeMinutes, overtime_multiplier: OVERTIME_MULTIPLIER, regular_amount: item.regularAmount, overtime_amount: item.overtimeAmount, total_amount: item.totalAmount, payment_reference: reference || null, note: "note" in item ? item.note || null : null }))).select("id, person_kind, person_id");
      if (itemsError || !itemRows) throw itemsError || new Error("Could not create payment items.");
...
function PayrollEntryDialog({ people, pending, method, reference, runDate, saving, onMethodChange, onReferenceChange, onRunDateChange, onSelect, onCancel, onSubmit }: { people: PayrollPersonOption[]; pending: PendingPayment[]; method: PaymentMethod; reference: string; runDate: string; saving: boolean; onMethodChange: (method: PaymentMethod) => void; onReferenceChange: (reference: string) => void; onRunDateChange: (date: string) => void; onSelect: (key: string) => void; onCancel: () => void; onSubmit: (values: PayrollFormValues) => void }) {
  ...
  return <div className="fixed inset-0 z-[150] flex items-center justify-center bg-black/50 p-4"><div className="card max-h-[90vh] w-full max-w-3xl overflow-y-auto p-6"><div className="mb-6 flex items-start justify-between gap-4"><div><p className="text-xs font-semibold uppercase tracking-[0.18em] text-primary">Manual payroll</p><h3 className="mt-1 font-display text-xl font-bold text-ink">Add payroll</h3><p className="mt-1 text-sm text-ink-muted">Completed unpaid hours are prefilled. You may adjust the payroll details before saving.</p></div><button type="button" className="btn-secondary px-3 py-2" onClick={onCancel}>Cancel</button></div><div className="grid gap-4 sm:grid-cols-2"><label className="label">Name<select className="input mt-1" value={activeKey} onChange={(event) => chooseStaff(event.target.value)}><option value="">Select staff</option>{pending.map((item) => <option key={item.key} value={item.key}>{item.personName}</option>)}</select></label><label className="label">Role<input className="input mt-1" value={role} onChange={(event) => setRole(event.target.value)} placeholder="Role" /></label><label className="label">Employment status<select className="input mt-1" value={employmentStatus} onChange={(event) => setEmploymentStatus(event.target.value as PendingPayment["employmentStatus"])}><option value="full_time">Full time</option><option value="part_time">Part time</option><option value="contract">Contract</option><option value="internship">Internship</option></select></label><label className="label">Payroll date<input type="date" className="input mt-1" value={runDate} onChange={(event) => onRunDateChange(event.target.value)} /></label><label className="label">Regular/Overtime hours<select className="input mt-1" value={hoursType} onChange={(event) => setHoursType(event.target.value as "regular" | "overtime")}><option value="regular">Regular hours</option><option value="overtime">Overtime hours</option></select></label><label className="label">{hoursType === "regular" ? "Regular hours" : "Overtime hours"}<input type="number" min="0" step="0.25" className="input mt-1" value={activeHours} onChange={(event) => hoursType === "regular" ? setRegularHours(Number(event.target.value)) : setOvertimeHours(Number(event.target.value))} /></label><label className="label">Rate<select className="input mt-1" value={rateType} onChange={(event) => setRateType(event.target.value as "normal" | "overtime")}><option value="normal">Normal hours</option><option value="overtime">Overtime rate</option></select></label><label className="label">{rateType === "normal" ? "Normal hourly rate" : "Overtime base rate"}<input type="number" min="0" step="0.01" className="input mt-1" value={baseRate} onChange={(event) => rateType === "normal" ? setNormalRate(Number(event.target.value)) : setOvertimeBaseRate(Number(event.target.value))} /><span className="mt-1 block text-xs text-ink-muted">{rateType === "overtime" ? `${money.format(baseRate)} × 2 = ${money.format(effectiveRate)}/hr` : `${money.format(effectiveRate)}/hr`}</span></label><label className="label">Payment method<select className="input mt-1" value={method} onChange={(event) => onMethodChange(event.target.value as PaymentMethod)}><option value="bank_transfer">Bank transfer</option><option value="mobile_money">Mobile money</option><option value="cash">Cash</option><option value="other">Other</option></select></label><label className="label">Reference <span className="font-normal text-ink-muted">(optional)</span><input className="input mt-1" value={reference} onChange={(event) => onReferenceChange(event.target.value)} placeholder="Transfer reference" /></label><label className="label sm:col-span-2">Note<textarea className="input mt-1 resize-none" rows={3} value={note} onChange={(event) => setNote(event.target.value)} placeholder="Optional payroll note" /></label></div><div className="mt-5 rounded-xl border border-primary/20 bg-primary/10 px-4 py-3"><p className="text-sm font-medium text-ink">Payment total: {money.format(amount)}</p><p className="mt-1 text-xs text-ink-muted">Regular: {regularHours}h × {money.format(normalRate)} · Overtime: {overtimeHours}h × {money.format(overtimeBaseRate)} × 2</p></div><div className="mt-6 flex justify-end gap-3"><button type="button" className="btn-secondary" onClick={onCancel}>Cancel</button><button type="button" className="btn-primary" disabled={saving || !active} onClick={() => active && onSubmit({ key: active.key, role, employmentStatus, regularHours, overtimeHours, normalRate, overtimeBaseRate, note })}><CreditCard className="h-4 w-4" />{saving ? "Adding…" : "Add payroll"}</button></div></div></div>;
}
